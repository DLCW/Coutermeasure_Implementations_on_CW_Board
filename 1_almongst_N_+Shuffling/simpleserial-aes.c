...
void aes_cipher()
{
    ...
    for(i=0; i<16; i++){
    	for (j=0; j<4; j++){
    		if (j == index[i]){
				tmp_1 = state[permIndices[i]] ^ rk[permIndices[i]] ;	
				state[permIndices[i]] = Sbox[tmp_1];
				}
    		else{
				tmp_1 = state_fake[permIndices[i]] ^ rk_fake[permIndices[i]] ;	
				state_fake[permIndices[i]] = Sbox[tmp_1];
				}
    	}
    }
